...
function PyVTK(dataset::VTKUnstructuredData)
    ...
    if dim(dataset) == 2
        points.SetData(julia_to_vtk([dataset.point_coords; zeros(num_of_points(dataset))']))
    else
        points.SetData(julia_to_vtk(dataset.point_coords))
    end
    ...
    _cells = julia_to_vtk(jl_cells, 0, 12) #vtkTypeIdArray() of serialized cell connectivity
    ncells = num_of_cells(dataset)
    vtkCells = vtk.vtkCellArray()
    vtkCells.SetCells(ncells, _cells)
    vtk_cell_types = julia_to_vtk(UInt8.(dataset.cell_types), 0, 3) #vtkUnsignedCharArray() of cell types 
    vtk_cell_locs = julia_to_vtk(jl_cell_locs, 0, 12) #vtkTypeIdArray() of cell locations
    grid.SetCells(vtk_cell_types, vtk_cell_locs, vtkCells)

    for m in keys(dataset.point_data)
        vtk_data_array = julia_to_vtk(dataset.point_data[m])
        vtk_data_array.SetName(m)
        grid.GetPointData().AddArray(vtk_data_array)
    end

    for m in keys(dataset.cell_data)
        vtk_data_array = julia_to_vtk(dataset.cell_data[m])
        vtk_data_array.SetName(m)
...
function PyVTK(dataset::VTKPolyData)
    ...
    points = vtk.vtkPoints()
    if dim(dataset) == 2
        vtktemp = julia_to_vtk([dataset.point_coords; zeros(num_of_points(dataset))'])
        points.SetData(vtktemp)
    else
        vtktemp = julia_to_vtk(dataset.point_coords)
        points.SetData(vtktemp)
    end
    ...
    _verts = julia_to_vtk(jl_verts, 0, 12) #vtkTypeIdArray() of serialized cell connectivity
    nverts = length(vertinds)
    verts = vtk.vtkCellArray()
    verts.SetCells(nverts, _verts)
    polydata.SetVerts(verts)

    _lines = julia_to_vtk(jl_lines, 0, 12) #vtkTypeIdArray() of serialized cell connectivity
    nlines = length(lineinds)
    lines = vtk.vtkCellArray()
    lines.SetCells(nlines, _lines)
    polydata.SetLines(lines)

    _polys = julia_to_vtk(jl_polys, 0, 12) #vtkTypeIdArray() of serialized cell connectivity
    npolys = length(polyinds)
    polys = vtk.vtkCellArray()
    polys.SetCells(npolys, _polys)
    polydata.SetPolys(polys)

    _strips = julia_to_vtk(jl_strips, 0, 12) #vtkTypeIdArray() of serialized cell connectivity
    ...
    for m in keys(dataset.cell_data)
        ...
        if _var_dim == 1
            ...
            vtk_data_array = julia_to_vtk(new_cell_data)
        else
...
function PyVTK(dataset::VTKStructuredData)
    ...
    if dim(dataset) == 2
        points.SetData(julia_to_vtk([_point_coords; zeros(num_of_points(dataset))']))
    else
        points.SetData(julia_to_vtk(_point_coords))
    end
    grid.SetPoints(points)
...
function PyVTK(dataset::VTKRectilinearData)
    ...
    xCoords = julia_to_vtk(dataset.point_coords[1])
    yCoords = julia_to_vtk(dataset.point_coords[2])
    if _dim == 2
        zCoords = julia_to_vtk([0.])
    else
        zCoords = julia_to_vtk(dataset.point_coords[3])
    end
...
function add_structured_data_to_grid!(grid, dataset::AbstractVTKStructuredData)
    for m in keys(dataset.point_data)
        ...
        vtk_data_array = julia_to_vtk(data_array)
        vtk_data_array.SetName(m)
        grid.GetPointData().AddArray(vtk_data_array)
    end
